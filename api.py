...
def configure_ngrok():
    ngrok_token = "2pycj7OxEtsOqzFioGZokfbBIVf_K2gwievMv7VPe8FqWw9p"
    try:
...
def update_soul_txt(public_url):
    try:
        with open("jony2.txt", "w") as file:
            file.write(public_url)
        logging.info(f"New ngrok link saved in jony2.txt")
    except Exception as e:
...
def update_vps_soul_txt(public_url):
    ...
    try:
        ...
        sftp = ssh.open_sftp()
        with sftp.open("jony2.txt", "w") as file:
            file.write(public_url)
        sftp.close()
        ssh.close()
        logging.info("Updated jony2.txt on VPS successfully.")
    except Exception as e:
        logging.error(f"Failed to update jony2.txt on VPS: {str(e)}")
